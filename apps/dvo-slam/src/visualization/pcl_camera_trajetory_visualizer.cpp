...
private:
  Option visibility_;
  std::shared_ptr<dvo::visualization::AsyncPointCloudBuilder::BuildJob> cloud_;
  std::mutex m_;
};
...
{
public:
  typedef std::shared_ptr<PclCameraVisualizer> PclCameraVisualizerPtr;
  typedef std::shared_ptr<PclTrajectoryVisualizer> PclTrajectoryVisualizerPtr;
  ...
  PclCameraTrajectoryVisualizerImpl(bool render_thread = true)
  {
    if(render_thread)
    {
      visualizer_thread_.reset(new std::thread(std::bind(&PclCameraTrajectoryVisualizerImpl::execVisualizerThread, this)));
  ...
  void render(int milliseconds)
  {
    ...
    if(visualizer_thread_ && visualizer_thread_->get_id() != std::this_thread::get_id())
    {
    ...
    // sync visualizer
    std::mutex::scoped_lock lock(sync_);
...
private:
  ...
  std::shared_ptr<std::thread> visualizer_thread_;
  std::mutex m_;
...
std::mutex& PclCameraTrajectoryVisualizer::sync()
{
  return impl_->sync_;
}
